        ...
        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            @foreach(['ikhtisar' => 'Ikhtisar & Lokasi', 'stok' => 'Monitoring Stok', 'maintenance' => 'Jadwal Maintenance', 'pengadaan' => 'Pengadaan Baru'] as $key => $label)
                <button type="button" wire:click="setTab('{{ $key }}')" 
                    class="{{ $activeTab === $key ? 'border-blue-500 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm transition-colors uppercase tracking-wider relative">
                    {{ $label }}
                    <div wire:loading wire:target="setTab('{{ $key }}')" class="absolute inset-0 bg-white/50 flex items-center justify-center">
