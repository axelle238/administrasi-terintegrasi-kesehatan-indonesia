<div class="animate-fade-in" x-data="{
    ...
    <!-- Fixed Header Area -->
    <div class="sticky top-[-2rem] lg:top-[-2rem] z-40 -mx-6 lg:-mx-8 mb-8 px-6 lg:px-8 py-4 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all duration-300">
        <div class="flex flex-col lg:flex-row justify-between items-center gap-4 max-w-[1600px] mx-auto">
            ...
            <div class="flex items-center gap-4 w-full lg:w-auto flex-1">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-violet-700 flex items-center justify-center text-white shadow-lg shadow-indigo-500/20 shrink-0">
                ...
                <div class="flex-1 max-w-xl">
                    <div class="flex items-center gap-2">
                        <span class="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em]">Nama Peran / Role</span>
                    </div>
                    <input wire:model="name" type="text" placeholder="Masukkan nama role..." 
                           class="w-full bg-transparent border-0 border-b border-slate-200 focus:border-indigo-500 focus:ring-0 text-lg font-black text-slate-800 placeholder-slate-300 px-0 py-0 transition-all">
                    @error('name') <span class="text-[10px] font-bold text-rose-500 block mt-0.5">{{ $message }}</span> @enderror
                </div>
            </div>

            <!-- Actions Section -->
            <div class="flex items-center gap-2 w-full lg:w-auto justify-end">
                <button wire:click="syncPermissions" wire:loading.attr="disabled" 
                        class="p-2.5 bg-slate-50 text-slate-500 rounded-xl border border-slate-200 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-100 transition-all group" title="Scan Fitur Baru">
                    <svg wire:loading.remove class="w-5 h-5 group-hover:rotate-180 transition-transform duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                    <svg wire:loading class="w-5 h-5 animate-spin text-indigo-600" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                </button>
                
                <div class="h-8 w-px bg-slate-200 mx-1 hidden sm:block"></div>

                <a href="{{ route('system.role.index') }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-slate-50 transition-all">
                    Batal
                </a>
                
                <button wire:click="save" class="px-8 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-wider hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:-translate-y-0.5 active:translate-y-0">
                    Simpan Role
                </button>
            </div>
        </div>
    ...
    <div class="px-0">
        @if(empty($groupedPermissions))
            <div class="flex flex-col items-center justify-center py-32 bg-white rounded-[3rem] border border-dashed border-slate-200 text-center">
                ...
                <h3 class="text-xl font-black text-slate-800">Database Fitur Belum Terisi</h3>
                <p class="text-slate-400 max-w-xs mt-2 mb-8">Klik tombol di bawah untuk memindai seluruh fitur dan fungsi yang tersedia di dalam sistem secara otomatis.</p>
                <button wire:click="syncPermissions" class="px-8 py-3 bg-indigo-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 transition-all">
                    Jalankan Sinkronisasi Fitur
                </button>
            </div>
        @else
            <div class="flex flex-col lg:flex-row gap-8 items-start">
                ...
                <div class="w-full lg:w-72 shrink-0">
                    <div class="bg-white rounded-[2rem] p-3 shadow-sm border border-slate-100 lg:sticky lg:top-32 overflow-hidden">
                        <div class="p-2 mb-3">
                            <div class="relative">
                        ...
                        <div class="max-h-[50vh] overflow-y-auto custom-scrollbar px-2 pb-2 space-y-1">
                            <button @click="activeGroup = 'all'" 
                                    :class="activeGroup === 'all' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'"
                ...
                <div class="flex-1 w-full min-w-0">
                        ...
                        <div x-show="(activeGroup === 'all' || activeGroup === '{{ $group }}') && (search === '' || '{{ strtolower($group) }}'.includes(search.toLowerCase()) || $el.querySelectorAll('label').length > 0)" 
                            ...
                            <div class="px-6 py-4 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100 flex justify-between items-center">
                                <div class="flex items-center gap-3">
                                    ...
                                    <h3 class="font-black text-slate-700 text-xs uppercase tracking-widest">{{ $group ?: 'Umum' }}</h3>
                                </div>
                    ...
                    <div x-show="search !== '' && $el.querySelectorAll('div[x-show]:not([style*=\'display: none\'])').length === 0" 
                         style="display: none;"
                         class="text-center py-20 bg-slate-50 rounded-[3rem] border border-dashed border-slate-200">
                        <p class="text-slate-400 text-sm font-bold italic text-glow">Hasil pencarian untuk "<span x-text="search"></span>" tidak ditemukan.</p>
                    </div>
                </div>
            </div>
        @endif
