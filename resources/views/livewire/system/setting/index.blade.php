<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 space-y-6" 
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div class="flex flex-wrap gap-2">
            <button wire:click="clearCache" wire:loading.attr="disabled" type="button" class="inline-flex items-center px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-200 transition-colors">
                ...
                <svg wire:loading target="clearCache" class="animate-spin w-4 h-4 mr-2" style="display: none;" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                Bersihkan Cache
            </button>
            <button wire:click="backupNow" wire:loading.attr="disabled" type="button" class="inline-flex items-center px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-200 transition-colors">
                <svg wire:loading.remove target="backupNow" class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" /></svg>
                <svg wire:loading target="backupNow" class="animate-spin w-4 h-4 mr-2" style="display: none;" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                Backup Manual
            </button>
        </div>
    </div>
            ...
            @foreach($schema as $key => $group)
                <button type="button" @click="activeTab = '{{ $key }}'"
                    ...
                    @if($group['icon'] == 'office-building')
                        <svg class="w-5 h-5" :class="activeTab === '{{ $key }}' ? 'text-white' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                    @elseif($group['icon'] == 'desktop-computer')
                        <svg class="w-5 h-5" :class="activeTab === '{{ $key }}' ? 'text-white' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                    @elseif($group['icon'] == 'clock')
                        <svg class="w-5 h-5" :class="activeTab === '{{ $key }}' ? 'text-white' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    ...
                    @elseif($group['icon'] == 'document-text')
                        <svg class="w-5 h-5" :class="activeTab === '{{ $key }}' ? 'text-white' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
        ...
        <div class="lg:col-span-3 bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
            <form wire:submit.prevent="save" class="space-y-6">
                ...
                @foreach($schema as $tabKey => $group)
                    <div x-show="activeTab === '{{ $tabKey }}'" x-cloak x-transition:enter.duration.300ms>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach($group['fields'] as $fieldKey => $config)
                                <div class="col-span-1 {{ in_array($config['type'], ['textarea', 'section_title']) ? 'md:col-span-2' : '' }}">
                                    ...
                                    @if($config['type'] === 'text')
                                        <input type="text" id="{{ $fieldKey }}" wire:model.defer="form.{{ $fieldKey }}" 
                                    ...
                                    @elseif($config['type'] === 'number')
                                        <input type="number" id="{{ $fieldKey }}" wire:model.defer="form.{{ $fieldKey }}" 
                                    ...
                                    @elseif($config['type'] === 'email')
                                        <input type="email" id="{{ $fieldKey }}" wire:model.defer="form.{{ $fieldKey }}" 
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium text-slate-800">
                                    
                                    @elseif($config['type'] === 'password')
                                        <input type="password" id="{{ $fieldKey }}" wire:model.defer="form.{{ $fieldKey }}" 
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium text-slate-800">

                                    @elseif($config['type'] === 'time')
                                        <input type="time" id="{{ $fieldKey }}" wire:model.defer="form.{{ $fieldKey }}" 
                                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium text-slate-800">

                                    @elseif($config['type'] === 'textarea')
                                        <textarea id="{{ $fieldKey }}" wire:model.defer="form.{{ $fieldKey }}" rows="3"
                                    ...
                                    @elseif($config['type'] === 'select')
                                        <div class="relative">
                                            <select id="{{ $fieldKey }}" wire:model.defer="form.{{ $fieldKey }}" 
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100 mt-6">
                    <div wire:loading wire:target="save" class="text-sm text-slate-400">Menyimpan konfigurasi...</div>
                    <button type="submit" wire:loading.attr="disabled" class="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/30 flex items-center gap-2">
                        <svg wire:loading.remove wire:target="save" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                        <svg wire:loading wire:target="save" class="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        Simpan Perubahan
                    </button>
                </div>
            </form>
