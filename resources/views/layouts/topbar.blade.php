<header class="flex items-center justify-between px-8 py-5 bg-white/90 backdrop-blur-xl border-b border-dashed border-slate-200 sticky top-0 z-40 transition-all duration-300">
    <div class="flex items-center gap-4">
        ...
        <button @click="sidebarOpen = !sidebarOpen" class="p-2 text-slate-500 focus:outline-none md:hidden hover:bg-slate-100 hover:text-blue-600 rounded-xl transition-all shadow-sm">
        ...
        <div class="hidden md:flex flex-col">
            <h1 class="text-xl font-black text-slate-800 tracking-tight leading-tight font-display">
                @if(isset($header))
                    {{ $header }}
                @else
                    Ringkasan Eksekutif
    ...
    <div class="flex items-center gap-4 md:gap-6">
        ...
        <div class="hidden lg:flex relative group">
            ...
            <input type="text" class="block w-72 pl-11 pr-4 py-2.5 border border-slate-200 rounded-xl leading-5 bg-slate-50/50 text-slate-700 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 sm:text-sm transition-all shadow-sm hover:bg-white font-medium" placeholder="Cari data pasien, dokter...">
        </div>
        ...
        <div x-data="{ dropdownOpen: false }" class="relative">
            <button @click="dropdownOpen = ! dropdownOpen" class="flex items-center gap-3 p-1.5 pr-4 rounded-full hover:bg-white hover:shadow-lg hover:shadow-slate-200/50 transition-all border border-transparent hover:border-slate-100">
                <div class="w-10 h-10 overflow-hidden rounded-full shadow-md border-2 border-white bg-gradient-to-tr from-blue-500 to-indigo-600 flex items-center justify-center text-white font-black text-sm">
                    {{ substr(Auth::user()->name, 0, 1) }}
                </div>
                <div class="hidden md:flex flex-col items-start">
                    <span class="text-sm font-bold text-slate-700 leading-none">{{ Str::limit(Auth::user()->name, 15) }}</span>
                    <span class="text-[10px] text-blue-600 font-bold uppercase tracking-wide mt-1">{{ Auth::user()->role }}</span>
                </div>
            ...
            <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95 translate-y-2" x-transition:enter-end="transform opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="transform opacity-100 scale-100 translate-y-0" x-transition:leave-end="transform opacity-0 scale-95 translate-y-2" class="absolute right-0 z-50 w-64 mt-3 origin-top-right bg-white rounded-2xl shadow-xl ring-1 ring-black/5 divide-y divide-slate-100 focus:outline-none overflow-hidden" style="display: none;">
                ...
                <div class="p-2">
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-slate-600 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-colors group">
                        <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                        Profil & Akun
                    </a>
                    <!-- Settings Link (Conditional/Optional) -->
                    @if(Auth::user()->role === 'admin')
                    <a href="{{ route('system.setting.index') }}" class="flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-slate-600 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-colors group">
